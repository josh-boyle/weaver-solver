import words
dictionary = words.get_words(100000)
words.print_dictionary(dictionary)
def bfs(paths: dict[str, dict[str, int | list[str]]], goal: str):
    ...
    for path in paths:
        ...
        for neighbor in words.neighbor_list_x(path, dictionary):
...
def bidir_bfs(top: dict[str, dict[str, int | list[str]]],bottom: dict[str, dict[str, int | list[str]]]):
    ...
    for word,meta in expanding.items():
        ...
        for neighbor in words.neighbor_list_x(word, dictionary):
            if neighbor not in new_paths:
